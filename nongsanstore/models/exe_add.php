...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST["name_sp"])) {
        $name_sp_Err= "Name is required";
    } else {
        $name_sp = test_input($_POST["name_sp"]);
        if (strlen($name_sp) < 5 || strlen($name_sp) > 40) {
            $name_sp_Err= "Length of name must be between 5 and 40 characters";
        }
    }

    //Type name
    if (empty($_POST["type"])) {
        $type_Err= "Name is required";
    } else {
        $type = test_input($_POST["type"]);
        if (strlen($type) < 5 || strlen($type) > 40) {
            $type_Err= "Length of name must be between 5 and 40 characters";
        }
    }
    ...
    if (!empty($_POST["description"])) {
        ...
        if (strlen($description) > 5000) {
            $description_Err = "Length of description must be less than 5000 characters";
        }
    }
    ...
    if (empty($_POST["price"])) {
        ...
        if (!filter_var($price, FILTER_VALIDATE_FLOAT) || $price < 0) {
            $price_Err = "Price must be a float and greater than 0";
        }
    }

    // Price_km
     if (empty($_POST["price_km"])) {
        $price_km_Err = "Price is required";
    } else {
        $price_km = test_input($_POST["price_km"]);
        if (!filter_var($price_km, FILTER_VALIDATE_FLOAT) || $price_km < 0) {
            $price_km_Err = "Price must be a float and greater than 0";
        }
    }
    ...
    if (!empty($_POST["image"])) {
        ...
        if (strlen($image) > 255) {
            $image_Err = "Length of the link image must be less or equal 255 characters";
        }
    }
    ...
    if (!empty($_POST["dcrip_quick"])) {
        ...
        if (strlen($dcrip_quick) > 5000) {
            $dcrip_quick_Err = "Length of dcrip_quick must be less than 5000 characters";
        }
    }
}
...
if ($name_sp_Err == "" && $type_Err == "" && $description_Err == "" && $price_Err == "" && $price_km_Err == "" && $dcrip_quick_Err == "" && $image_Err == "") {
    $sql = "INSERT INTO tb_sanpham VALUES (NULL, '$name_sp', '$type','$description', '$price', '$price_km','$dcrip_quick','$image',NULL)";
